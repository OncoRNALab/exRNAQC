...
sample_annotation<-read_csv("Annotation exRNAQC004.csv")
...
spike_conc_ERCC<-read_tsv(here::here("Resources", "cms_095046.txt"))
...
spike_conc_Seq<-read_tsv(here::here("Resources", "RNAsequins_isoform_mix.v2.2.tsv"))
colnames(spike_conc_Seq)<-c("spike_id","length","mix1","mix2")
spike_conc_Seq<-spike_conc_Seq[,c("spike_id","mix1")]

spike_conc_gene_Seq<-read_tsv(here::here("Resources", "RNAsequins_gene_mix.v2.2.tsv"))
...
```{r}
#The full analysis was done for all the replicates, but only the following two replicates were included in the final article. 
Norgen_samples <- c("RNA003614L1", "RNA003616L1")
#to make the linear correlation of 
#Deduplicated files loadthe doublePos_Dedup.csv 
#Non deduplicated files doublePos_NoDedup.csv
double_positives_FC<-read_csv2(paste0(data_path,"doublePos_Dedup.csv"))
                               
test <- sample_annotation %>% dplyr::filter(UniqueID %in% Norgen_samples)

single_pos_FC <- data.frame(Condition = rep(unique(test$GraphKit),1) %>% sort(), 
                          Replicates = rep(c("RNA1-RNA3"), length(unique(test$GraphKit))), 
                          SP_no_filter = NA, DP_no_filter = NA, DN_no_filter = NA,   
                          SP_after_filter = NA, DP_after_filter = NA, DN_after_filter = NA, filter_threshold = NA) 
single_pos_FC$full_name <- paste(single_pos_FC$Condition, single_pos_FC$Replicates, sep="_")
```

```{r}
for (UniqueCon in unique(test$GraphKit)){
  sample_duplicates_2<-test %>% filter(GraphKit==UniqueCon) %>%
    dplyr::select(UniqueID,GraphKit,TechnicalReplicate)
  
  if(nrow(sample_duplicates_2)>1){
    #print(UniqueKit)
    double_pos_sample2<-double_positives_FC %>%
        dplyr::select(ensembl_gene_id, sample_duplicates_2$UniqueID)
    
    samples_comb2 <- combn(sample_duplicates_2$UniqueID ,2) #compare 2 of the 3 samples at a time
    
    for (n_col in 1:ncol(samples_comb2)){
      #print(samples_comb[,n_col])
      samplename1 <- samples_comb2[,n_col][1]
      sample1 <- test[test$UniqueID==samplename1,]$TechnicalReplicate
      samplename2 <- samples_comb2[,n_col][2]
      sample2 <- test[test$UniqueID==samplename2,]$TechnicalReplicate
      varname <- paste0(UniqueCon,"_",sample1,"-",sample2)
      #print(varname)
      
      double_pos_subset <- double_pos_sample2 %>% dplyr::select(ensembl_gene_id, paste(samplename1), paste(samplename2))
      ...
      single_pos_FC[single_pos_FC$full_name==paste(varname),]$filter_threshold <- threshold
      
      #Single Positives
      single_pos_FC[single_pos_FC$full_name==paste(varname),]$SP_no_filter <- sum( 
        ((double_pos_subset[,paste(samplename1)] > 0) & (double_pos_subset[,paste(samplename2)] == 0)) | 
          ((double_pos_subset[,paste(samplename1)] == 0) & (double_pos_subset[,paste(samplename2)] > 0))
        )
      single_pos_FC[single_pos_FC$full_name==paste(varname),]$SP_after_filter <-  sum( 
      ...
      #Double Positives
      single_pos_FC[single_pos_FC$full_name==paste(varname),]$DP_no_filter <- sum((double_pos_subset[,paste(samplename1)] > 0) & 
                                                                              (double_pos_subset[,paste(samplename2)] > 0))
      single_pos_FC[single_pos_FC$full_name==paste(varname),]$DP_after_filter <- sum((double_pos_subset[,paste(samplename1)] > threshold) & 
                                                                                 (double_pos_subset[,paste(samplename2)] > threshold))
      
      #Double Negatives
      single_pos_FC[single_pos_FC$full_name==paste(varname),]$DN_no_filter <- sum((double_pos_subset[,paste(samplename1)] == 0) & 
                                                                              (double_pos_subset[,paste(samplename2)] == 0))
      single_pos_FC[single_pos_FC$full_name==paste(varname),]$DN_after_filter <- sum((double_pos_subset[,paste(samplename1)] <= threshold) & 
                                                                              (double_pos_subset[,paste(samplename2)] <= threshold))
      
      #Calculate percentages of SP and DP remaining after filtering
      single_pos_FC$remainingSP <- single_pos_FC$SP_after_filter/single_pos_FC$SP_no_filter
      single_pos_FC$remainingDP <- single_pos_FC$DP_after_filter/single_pos_FC$DP_no_filter
      
      lm_tmp <- lm(log(pull(double_pos_sample2,samplename1)+1,10) ~ -1 + log(pull(double_pos_sample2, samplename2)+1,10)) # fit linear model of log values technical replicates (no intercept)
      ...
      p <- ggplot(double_pos_subset, aes(x=log(get(samplename1)+1,10), y=log(get(samplename2)+1,10), col=colouring)) +
        ...
        labs(title=paste(varname, ": cutoff of", threshold),
             subtitle=paste0("% remaining after filtering: ",
                             round(single_pos_FC[single_pos_FC$full_name==paste(varname),]$remainingSP*100,2),"% of SP (R2 = ",
      ...
      print(p)
      ggsave(paste0(varname, "CorrDupl.pdf"), p, dpi = 300, width = 6, height = 6)
...
# Run this section of the code to make the  Kallisto counts after clumpify deduplication, have to provide the list of all kallisto outputs form the preprocessing pipeline.
# library(biomaRt)
...
#### Read in data tables previously made with commands above
```
...
```{r, echo=FALSE, fig.height=4.5, fig.width=8, fig.align="center", fig.keep="last", fig.cap="Number of protein coding genes that are detected. Left: all protein coding genes that are detected with at least one count; right: protein coding genes that are reproducibly detected (≥ precision threshold that eliminates 95% of single positives). (QIA1: QIAamp ccfDNA/RNA kit, 1 ml input; QIA4: QIAamp ccfDNA/RNA kit, 4 ml input; NOR0.25: plasma/serum circulating and exosomal RNA purification kit, 0.25 ml input; NOR5: plasma/serum circulating and exosomal RNA purification kit, 5 ml input; MAX0.1: Maxwell RSC miRNA plasma and exosome kit, 0.1 ml input; MAX0.5: Maxwell RSC miRNA plasma and exosome kit, 0.5 ml input; MIR0.2: miRNeasy serum/plasma kit, 0.2 ml input; MIRA0.2: miRNeasy serum/plasma advanced kit, 0.2 ml input; MIRA0.6: miRNeasy serum/plasma advanced kit, 0.6 ml input; MIRV0.1: mirVana PARIS kit, 0.1 ml input; MIRV0.625: mirVana PARIS kit, 0.625 ml input; NUC0.3: NucleoSpin miRNA plasma kit, 0.3 ml; NUC0.9: NucleoSpin miRNA plasma kit, 0.9 ml input)"}
