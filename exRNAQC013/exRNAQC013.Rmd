...
miRNAs <- data.table::fread(paste0(data_path, "allmiRs.txt"), header=T, data.table = F) #download count file from ArrayExpress.
spikes <- data.table::fread(paste0(data_path, "allspikes.txt"), header=T, data.table = F) 
```
...
mapped_df<-read_tsv(paste0(data_path, "smallRNA_table.tsv"))
mapped_df_melt <-mapped_df %>% melt(value.name = "reads")
mapped_df_melt_annot <- full_join(mapped_df_melt, sample_annotation, by = "UniqueID")
mapped_df_annot <- merge(sample_annotation, mapped_df, by = "UniqueID")
```
